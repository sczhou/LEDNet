...
class MyDownSample(nn.Module):
    def __init__(self, in_channels, out_channels, bias=True):
        ...
        self.down = nn.Sequential(nn.Conv2d(in_channels, out_channels, 1, stride=1, padding=0, bias=bias),
                                nn.LeakyReLU(0.2, True),
                                nn.Conv2d(out_channels, out_channels, 3, stride=2, padding=1, bias=bias),
                                nn.LeakyReLU(0.2, True),
...
class MyUpSample(nn.Module):
    def __init__(self, in_channels, out_channels, bias=True):
        ...
        self.up = nn.Sequential(nn.Conv2d(in_channels, in_channels, 3, stride=1, padding=1, bias=bias),
                                nn.LeakyReLU(0.2, True),
...
class LEDNet2(nn.Module):
    def __init__(self, channels=[32, 64, 128, 128], connection=False):
        ...
        self.E_block1 = nn.Sequential(
            nn.Conv2d(3, ch1, 3, stride=1, padding=1), nn.LeakyReLU(0.2, True),
